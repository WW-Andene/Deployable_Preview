(function () {
  ...
  Studio.renderInspector = function (root) {
    ...
    // ── Position (Canva-style free placement, filled in once an element
    //    has been dragged at least once, converting it to absolute) ──
    root.appendChild(h("div", { class: "st-section-title", text: "Position" }));
    if (c.position === "absolute" || c.position === "fixed") {
      var posGrid = h("div", { class: "st-grid-4" });
      posGrid.appendChild(numField("Top", c.top, "px", function (v) { Studio.guides.setStyle("top", v); }));
      posGrid.appendChild(numField("Right", c.right, "px", function (v) { Studio.guides.setStyle("right", v); }));
      posGrid.appendChild(numField("Bottom", c.bottom, "px", function (v) { Studio.guides.setStyle("bottom", v); }));
      posGrid.appendChild(numField("Left", c.left, "px", function (v) { Studio.guides.setStyle("left", v); }));
      root.appendChild(posGrid);
      root.appendChild(numField("Layer (z-index)", c.zIndex === "auto" ? 0 : c.zIndex, "", function (v) { Studio.guides.setStyle("zIndex", v); }));
      if (sel.leftFlow) {
        root.appendChild(h("div", { class: "st-hint", text: "This element was freed from its " + (sel.parentLayout || "layout") + " flow — its former siblings reflowed to fill the gap." }));
      }
    } else {
