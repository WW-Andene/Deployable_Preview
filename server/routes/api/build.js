...
router.post("/webhook", (req, res) => {
  ...
  // I2: auto PR previews. Honor the per-repo `autoPRPreviews` opt-in.
  // - opened/reopened/synchronize → ensure the PR head branch is in
  //   activeBranches, then build it.
  // - closed → remove the branch (works whether the PR was merged or
  //   discarded; the branch was DV-managed for the lifetime of the PR).
  if (event === "pull_request" && repoConfig.autoPRPreviews) {
    ...
    if (action === "opened" || action === "reopened" || action === "synchronize") {
      ...
      if (!bc) {
        ...
        try { require("../../config").saveConfig(); } catch (_) {}
      }
      Promise.resolve(deployBranch(repoConfig, bc)).catch((e) => {
